...
function Marker({
  ...
  const initials = useMemo(() => {
    ...
    return marker.label.substring(0, 2).toUpperCase();
  }, [marker.label]);

  const bgColor = useMemo(() => {
    if (!marker.label) return "#10b981";
    const colors = [
      "#ef4444", "#f97316", "#f59e0b", "#84cc16", "#10b981", 
      "#06b6d4", "#3b82f6", "#6366f1", "#8b5cf6", "#d946ef", "#f43f5e"
    ];
    let hash = 0;
    for (let i = 0; i < marker.label.length; i++) {
      hash = marker.label.charCodeAt(i) + ((hash << 5) - hash);
    }
